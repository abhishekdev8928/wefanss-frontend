...
const SectionMasterList = () => {
  ...
  const handleChange = async (currentStatus, id) => {
    ...
    try {
      const res_data = await updatesectionmasterStatus(id, newStatus);
      ...
      toast.success("SectionMaster Master status updated successfully");
      fetchData();
    } catch (error) {
  ...
  const fetchData = async () => {
    try {
      ...
      setSectionMasterList(result.msg || []);
    } catch (error) {
  ...
  const handleYesNo = async () => {
    ...
    try {
      const data = await deletesectionmaster(deleteId);
      ...
      toast.success("SectionMaster  deleted successfully");
      setSectionMasterList((prevItems) =>
        prevItems.filter((row) => row._id !== deleteId)
      );
      setModalOpen2(false);
      setDeleteId(null);
    } catch (error) {
      console.error("Error deleting  SectionMaster:", error);
